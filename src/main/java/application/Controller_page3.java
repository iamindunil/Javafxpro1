...
public class Controller_page3 {
    ...
    public void create_list_movies() {
        ...
        try (Connection connection = DriverManager.getConnection(connectionUrl)) {
            ...
            String sql2 = "SELECT count(*) FROM (\n" +
                    "    SELECT TVS_name, imdb_ratings, ROW_NUMBER() OVER (ORDER BY TVS_name) AS mid FROM Animetvseries\n" +
                    "    UNION\n" +
                    "    SELECT TVS_name, imdb_ratings, ((select count(*) from Animetvseries) + (ROW_NUMBER() OVER (ORDER BY TVS_name))) AS mid FROM Lactiontvseries\n" +
                    ") AS subquery;";
        ...
        if(!flag){
            ...
            try {
                for (int i = 0; i < count; i++) {
                    ...
                    anchorPane1.getChildren().add(line);
                    ...
                    Y += 10;
                    anchorPane1.getChildren().add(line2);
                    flag = true;
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void switchtoinfo2(String name) throws IOException {
        showinfo2(name);
        System.out.println(name);
    }
    public void showinfo2(String name) throws IOException {
        Con3 con3 = new Con3();
        con3.setname(name);
        con3.Createstg();
    }
